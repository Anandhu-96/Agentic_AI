"""Regenerate the GitHub Pages showcase site under ``docs/``.

GitHub Pages is configured to deploy from ``main:/docs``, so this script keeps
the static site in sync with the dashboard preview images. Run it after
regenerating ``dashboard/detection_output.jpg`` and commit the result:

    python scripts/build_showcase.py

It is idempotent: it only (re)writes files under ``docs/``.
"""
...
<div class="wrap">
  ...
  <div class="img-row">
    <figure>
      <img src="detection_output.jpg" alt="Annotated detection frame with person polygons, machine danger zone and geofences" />
      <figcaption>Detection overlay &mdash; person polygons, machine danger zone, static geofences</figcaption>
    </figure>
    <figure>
      <img src="geofence_test.jpg" alt="Geofence zone visualization" />
      <figcaption>Restricted-zone geofencing</figcaption>
    </figure>
  </div>

  <p>
    <a class="btn" href="isip-dashboard.html">Open interactive dashboard demo</a>
...
def main() -> None:
    ...
    (DOCS / "index.html").write_text(LANDING_PAGE, encoding="utf-8")
    print(f"showcase updated under {DOCS}")
